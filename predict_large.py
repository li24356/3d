...
checkpoints_root = Path('checkpoints4')
...
def main():
    ...
    if model_name == 'aerb_pro':
        ...
        # 从 checkpoint 里读取 base_channels 配置
        ckpt_bc = None
        if isinstance(checkpoint, dict):
            ckpt_bc = checkpoint.get('model_config', {}).get('base_channels')

        # 构造尝试序列，优先使用 ckpt 中的 base_channels
        candidates = []
        if ckpt_bc:
    ...
    print(f"  • 输出文件:")
    print(f"      - {out_prob_npy.name} (概率图, float32)")
    print(f"      - {out_mask_npy.name} (二值分割, uint8)")
